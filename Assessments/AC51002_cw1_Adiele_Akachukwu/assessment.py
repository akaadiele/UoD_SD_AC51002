...
def storeStaffData(staffId, operatingHours, previousHoursWorked):
    ...
    itemsPerHour = list(productsCataloguePerHour.values())
    totalItems = 0
...
def logIn(clockInTime):
    ...
    while productionTime <= int(workingHoursStop):
        ...
        if (currentOperatingHours >= operatingHoursMax):
...
# 5. Include in the software a set value for the number of items that can be produced in an hour 
# (e.g., 100 items/hour). This could be a fixed value.
# ---DONE---#
